...
每次 DDA 路径跨越格子边界时，系统 MUST 检查实际跨越的那条边是否被 `wall_block` 阻挡，包括离开格的出口边和进入格的入口边。

#### Scenario: 跨垂直边向右
- **WHEN** DDA 从格子 A 跨右边进入格子 B
- **THEN** 检查 A 的东墙位（bit 2）和 B 的西墙位（bit 8）
- **AND** 任一位被置位即视为阻挡

#### Scenario: 跨水平边向下
- **WHEN** DDA 从格子 A 跨下边进入格子 B
- **THEN** 检查 A 的南墙位（bit 4）和 B 的北墙位（bit 1）
- **AND** 任一位被置位即视为阻挡

#### Scenario: 起点的离开方向有墙
- **WHEN** 子弹从起点向某方向射出，起点对应方向有 wall_block
